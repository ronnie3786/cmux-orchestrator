...
ENDPOINTS: list[dict[str, Any]] = [
    ...
    endpoint("POST", "/api/orchestrator-v2/cmux/sessions/{workspaceId}/kill", "Stop a cmux session and detach any task link.", category="Cmux", safety="terminal_write"),
    endpoint("POST", "/api/orchestrator-v2/cmux/sessions/{workspaceId}/restart", "Restart a cmux session with the same folder and launch type, relinking its task.", category="Cmux", safety="terminal_write"),
    endpoint("GET", "/api/orchestrator-v2/orphans", "List active unlinked cmux sessions.", category="Cmux"),
    endpoint("GET", "/api/orchestrator-v2/events/stream", "Server-sent events stream that fires whenever Orchestrator V2 state changes.", category="Orchestrator V2"),
    endpoint("GET", "/api/orchestrator-v2/events/token", "Read the current Orchestrator V2 state change token.", category="Orchestrator V2"),
