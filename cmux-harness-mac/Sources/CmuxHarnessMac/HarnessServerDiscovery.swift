...
enum HarnessServerDiscovery {
    private static var activeBrowsers: [UUID: BonjourHarnessBrowser] = [:]
...
@MainActor
private final class BonjourHarnessBrowser: NSObject, @preconcurrency NetServiceBrowserDelegate, @preconcurrency NetServiceDelegate {
    ...
    func start() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: max(0.5, timeout), repeats: false) { [weak self] _ in
            Task { @MainActor in
                self?.finish()
            }
        }
