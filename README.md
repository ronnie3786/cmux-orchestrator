...
http://localhost:9091/orchestrator-v2?view=voice # Voice Visual Mode (talking orchestrator)
```
...
A full-screen talking-orchestrator view lives at `/orchestrator-v2?view=voice` (or the Visual Mode button in the V2 top bar): push-to-talk conversation with the agent, live captions, spoken answers, and on-the-fly rich HTML answer panels. See `docs/ORCHESTRATOR_V2_VOICE_VISUAL_MODE.md` for the full spec and runbook. It uses local services configured in `.env.local`:

```text
ORCHESTRATOR_V2_STT_BACKEND=parakeet          # Parakeet STT (falls back to faster-whisper)
ORCHESTRATOR_V2_PARAKEET_URL=http://127.0.0.1:18793
ORCHESTRATOR_V2_TTS_BACKEND=kokoro            # Kokoro TTS (Piper/ElevenLabs remain available)
ORCHESTRATOR_V2_KOKORO_URL=http://127.0.0.1:8898
ORCHESTRATOR_V2_KOKORO_VOICE=bm_daniel
ORCHESTRATOR_V2_AGENT_PORT=8795               # Node sidecar port (default 8792; set when that port is taken)
```

Rich answer panels are generated by the Fireworks enrichment endpoint (`POST /api/orchestrator-v2/voice/enrich`) and rendered only after server-side validation inside a fully sandboxed iframe. The microphone requires a secure context, so use `http://localhost:9091` on the Mac itself (LAN/Tailscale IPs over plain HTTP cannot record).
